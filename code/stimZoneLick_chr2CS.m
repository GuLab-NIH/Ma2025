...
mouseUse=[27:44];
% idx=[1 2 3 6 7 10 11 12 13 15];
% mouseUse=mouseUse(idx);
for mouse=1:length(mouseUse);
    ...
    allLickAfter{mouse}{run}={};
        if ~isempty(stimZoneLengthAllData{mouseUse(mouse)}{run});%stimulation should be at least 14cm
            ...
            reward=rewardSegAllData{mouseUse(mouse)}{run};
...
for mouse=1:length(allLickStim)
    for run=1:length(allLickStim{mouse});
        ...
        for stim=1:length(allLickStim{mouse}{run});
            if length(allLickStim{mouse}{run}{stim})>=15/bin;
            allLickStimAll=[allLickStimAll;allLickStim{mouse}{run}{stim}(1:15/bin)];
            
            if length(allLickBefore{mouse}{run}{stim})>=15/bin;
            allLickBeforeAll=[allLickBeforeAll;allLickBefore{mouse}{run}{stim}(end-15/bin+1:end)];
            end
            if length(allLickAfter{mouse}{run}{stim})>=15/bin;
            allLickAfterAll=[allLickAfterAll;allLickAfter{mouse}{run}{stim}(1:15/bin)];
            end
            end
        end
        end
...
saveas(gcf,'chr2CSLick_beforeStimAfter1cm.fig')
