...
if position(thisStimIdx(1))>=startDis & position(thisStimIdx(end))<=endDis;
    ...
    allSpeedStim{mouse}{run}{nStim}=thisPosition(end)/(thisTime(end)*60);
    ...
    allSpeedBefore{mouse}{run}{nStim}=thisPositionBefore(end)/(thisTimeBefore(end)*60);
 ...
 stimEndPosition=position(thisStimIdx(end));%no subtraction by first one
    ...
    thisTimeAfter=thisTimeAfter-thisTimeAfter(1);
        allSpeedAfter{mouse}{run}{nStim}=thisPositionAfter(end)/(thisTimeAfter(end)*60);
...
allSpeedStimAllPerSession=[];
allSpeedAfterAllPerSession=[];
allSpeedBeforeAllPerSession=[];
...
beforeSession=[];
    ...
    for run=1:length(allSpeedStim{mouse});
        ...
        for stim=1:length(allSpeedStim{mouse}{run});
            if round(stimLength{mouse}{run}(stim))>10; %15 cm stimulations
                if ~isempty(allSpeedStim{mouse}{run}{stim})
            allSpeedStimAll=[allSpeedStimAll;allSpeedStim{mouse}{run}{stim}];
            yesOrNo(end+1,1)=1;
            stimSession=[stimSession;allSpeedStim{mouse}{run}{stim}];
            if ~isempty(allSpeedBefore{mouse}{run}{stim})
            allSpeedBeforeAll=[allSpeedBeforeAll;allSpeedBefore{mouse}{run}{stim}];
             yesOrNo(end,2)=1;
             beforeSession=[beforeSession;allSpeedBefore{mouse}{run}{stim}];
            else
                allSpeedBeforeAll=[allSpeedBeforeAll;nan];
                beforeSession=[beforeSession;nan];
                yesOrNo(end,2)=0;
            end
            if ~isempty(allSpeedAfter{mouse}{run}{stim})
            allSpeedAfterAll=[allSpeedAfterAll;allSpeedAfter{mouse}{run}{stim}];
            yesOrNo(end,3)=1;
            afterSession=[afterSession;allSpeedAfter{mouse}{run}{stim}];
            else
                allSpeedAfterAll=[allSpeedAfterAll;nan];
    ...
    allSpeedStimAllPerSession(mouse,1)=nanmean(stimSession);
allSpeedAfterAllPerSession(mouse,1)=nanmean(afterSession);
allSpeedBeforeAllPerSession(mouse,1)=nanmean(beforeSession);
end
allSpeedStimAll(isinf(allSpeedStimAll))=nan;
allSpeedAfterAll(isinf(allSpeedAfterAll))=nan;
allSpeedBeforeAll(isinf(allSpeedBeforeAll))=nan;

%find the stimulations with all components
useIdx=find(sum(yesOrNo,2)==3); 

allSpeedAfterAllUse=allSpeedAfterAll(useIdx,:);
allSpeedStimAllUse=allSpeedStimAll(useIdx,:);
allSpeedBeforeAllUse=allSpeedBeforeAll(useIdx,:);
save('gfpCSSpeedUseWholeZoneNoRemoveReward.mat','allSpeedAfterAllUse','allSpeedBeforeAllUse','allSpeedStimAllUse','allSpeedAfterAllPerSession','allSpeedBeforeAllPerSession','allSpeedStimAllPerSession')
...
% 
% saveas(gcf,'gfpRS_beforeStimAfter1cmRemoveReward.fig')
...
% title(['beforeStimAfter',num2str(bin),'cm']);
% 
% saveas(gcf,'gfpRS_beforeStimAfter2_5cm.fig')
% 
% 
% 
% 
