...
dataList = dir([fpath,'/Record*processed.mat']);
...
for dataIdx = 1:noData
    ...
    totalNoCh = 32;
    chList = 1:totalNoCh;%str2num(tmpChList{1}(3:end))%
        ...
        for freqNum = 1:length(stimFreq)
            for durNum = 1:length(stimDur)
                ...
                for stimRound = 1:length(stimIdx)
                    ...
                    ax = gca;
                    tmpData = spikeData.LFPTimeAmp{chIdx}{stimIdx(stimRound)};
                    ...
                    if(~isnan(fitResult.Params.k))
                        ...
                        lgd = legend(ax,num2str(tmpTimeConstant(stimIdx(stimRound))),num2str(fitResult.gof.adjrsquare), 'Location','south');
                        lgd.Box='off';
