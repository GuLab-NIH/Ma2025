...
% list=dir('/Users/kimj65/Documents/Data/MEC_optogenetics/AAV8-hsyn_ChR2/20211110/5MaxSNR_Ch/Record1*_processed.mat');
...
exZoneIdx = 224;
ex30ms =  30 / 1000 * sampleRate;
exZoneIdx1_2 = 224 + ex30ms;
exZoneIdx1_3 = exZoneIdx1_2 + ex30ms;
exZoneTime2 = 70; % (ms) exclude baseline for caculation of firing rate 80ms means using first 20 ms for stim part 1 (0 - 20 ms)
exZoneIdx2 = exZoneTime2 / 1000 * sampleRate;
exZoneIdx2_2 = exZoneIdx2 - ex30ms;
exZoneIdx2_3 = exZoneIdx2_2 - ex30ms;
...
for i = 1:noList
    ...
    for stimNum = 1%:length(stimIdx)
        ...
        for chIdx = 1:noCh
            ...
            tmpSpkBin(durStim, 5) = 1; % stim area 1 (0 - 30 ms) with artifact exclusion
            tmpSpkBin(durStim, 6) = 1; % stim area 2 (30 - 60 ms) with artifact exclusion
            tmpSpkBin(durStim, 7) = 1; % stim area 3 (60 - 90 ms) with artifact exclusion
            ...
            for pulseIdx = 1:noPulse%%
                ...
                tmpStim2_2 = [spikeData.stimON{stimIdx(stimNum)}(pulseIdx)-exZoneIdx2_2:spikeData.stimON{stimIdx(stimNum)}(pulseIdx)+exZoneIdx1_2] + tmpShift;%%
                tmpSpkBin(tmpStim2_2, 6) = 0;%%
                tmpStim2_3 = [spikeData.stimON{stimIdx(stimNum)}(pulseIdx)-exZoneIdx2_3:spikeData.stimON{stimIdx(stimNum)}(pulseIdx)+exZoneIdx1_3] + tmpShift;%%
                tmpSpkBin(tmpStim2_3, 7) = 0;%%
            ...
            spkDurStim2_2 = tmpSpkBin(:,1).*tmpSpkBin(:,6);
            spkDurStim2_3 = tmpSpkBin(:,1).*tmpSpkBin(:,7);
            ...
            meanSpkDurStim2 = sum(spkDurStim2_2) / (tmpPeriod(6) / sampleRate);
            meanSpkDurStim3 = sum(spkDurStim2_3) / (tmpPeriod(6) / sampleRate);
            meanSpkPostStim = sum(spkPostStim) / (tmpPeriod(4) / sampleRate);
            spikeData.Count.avgSpikeRate.(['ch',num2str(chList(chIdx)),'_',stimConditions{stimIdx(stimNum)}]) = [meanSpkPreStim, meanSpkDurStim, meanSpkDurStim2, meanSpkDurStim3, meanSpkPostStim];
...
for i=1:noData
    ...
    plot([1:5],avgData(i,:),'.-','Color',[0.5, 0.5, 0.5]);
    hold on;
end
...
err=errorbar([1:5],totavgData,totsteData,'Marker','.','Color','black','MarkerSize',20,'LineWidth',2,'CapSize',5);
ax=gca;
ax.XTick=[1:5];
ax.XLim = [0.7,5.3];
ax.XTickLabel = {'Pre','Stim1','Stim2','Stim3','Post'};
