...
idxGFPRS=[16:26];
idxGFPCS=[45:56];
allIdxAll={};%poor performer only  based on Yan's calcualtion: email on 4/10/25
allIdxAll{1}=setdiff([1:1:length(idxCHR2RS)],[1 2 3 5 10 12 14 15 8 13]);%CHR2RS %REMOVE 8 AND 13 with bad lick signal: with more than 500 licks per bin (spatial bin below)
allIdxAll{2}=setdiff([1:1:length(idxCHR2CS)],[1 2 3 14 15 12]); %chr2 CS: %REMOVE 12 AND 14 with bad lick signal: with more than 500 licks per bin (spatial bin below)
allIdxAll{3}=setdiff([1:1:length(idxGFPRS)],[]);%GFPRS
allIdxAll{4}=setdiff([1:1:length(idxGFPCS)],[12]); %GFP CS
...
saveas(gcf,'absPreRewardictiveLickingGoodRewardLick.fig')
...
ylim([0 9])
saveas(gcf,'RBRTogetherAbsBeforeRewardGoodRewardLick.fig')
...
ylim([0 17])
saveas(gcf,'RBRTogetherAbsBeforeRewardNormGoodRewardLick.fig')
...
saveas(gcf,'lickDistributionALLGoodRewardLick.fig')
...
saveas(gcf,'lickDistributionALL_noNormalizeGoodRewardLick.fig')
...
saveas(gcf,'lickDistributionALL_noNormalizeChangeGoodRewardLick.fig')
...
cueCenters=mean(cues,2);
edgeCenters=(edges(2:end)+edges(1:end-1))/2;
nearestBin=[];
for n=1:3;
    a=abs(edgeCenters-cueCenters(n));
    [~,nearestBin(n,1)]=min(a);
end
adjBin=5;%bin around the cue bin
aroundCueBin=[];
aroundCueBin=[nearestBin-adjBin nearestBin+adjBin];
...
hold on
line([adjBin adjBin]+0.5,[0 10])
saveas(gcf,'lickDistributionALL_noNormalizeChangeAvgSessionGoodRewardLick.fig')
...
saveas(gcf,'lickDistributionALL_NormalizeChangeAvgSessionGoodRewardLick.fig')
...
tightfig
saveas(gcf,'lickDistriRBRCHR2CSGoodRewardLick.fig')
...
title(['late runs',num2str(dayThresh+1),'to',num2str(size(countRBR,1))])
saveas(gcf,'lickDistriRBRCHR2RSGoodRewardLick.fig')
...
title(['late runs',num2str(dayThresh+1),'to',num2str(size(countRBR,1))])
tightfig
saveas(gcf,'lickDistriRBRGFPCSGoodRewardLick.fig')
...
title(['late runs',num2str(dayThresh+1),'to',num2str(size(countRBR,1))])
saveas(gcf,'lickDistriRBRGFPRSGoodRewardLick.fig')